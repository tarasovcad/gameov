    ...
    <Link href={"/games/" + item.title}>
      <Card
        ...
        className={`bg-white dark:bg-bg overflow-hidden border transition-colors duration-300 ease-in-out border-border/40 dark:hover:border-border hover:border-border relative group flex flex-col w-full h-full dark:hover:bg-bg/90 hover:bg-white/60 ${
          gamePage ? "max-[850px]:flex-row max-[850px]:items-center" : ""
        } ${!listView ? "flex-row items-center" : ""}`}>
        <div
          className={`aspect-[16/9] min-[1300px]:h-44 relative w-full max-[1129px]:aspect-[23/10] max-[1000px]:aspect-[23/9] max-[769px]:aspect-[23/8] max-[701px]:aspect-[10/3] overflow-hidden max-[600px]:aspect-[16/7] ${
            gamePage
              ? "max-[850px]:w-1/3 max-[650px]:w-1/2 max-[850px]:h-full max-[850px]:aspect-auto"
              : ""
          } ${!listView ? "w-1/3 h-full aspect-auto" : ""}`}>
          <Image
            src={item.image}
            alt={item.title}
            layout="fill"
          ...
          <div className="flex-grow">
            <p
              className={`dark:text-secondary_text line-clamp-2 text-[#464646] mb-6 text-sm max-[1150px]:text-[13px] max-[1150px]:mb-[20px] ${
                gamePage ? "max-[850px]:mb-2" : ""
              }`}>
              {item.description}
            </p>
          </div>
          <div
            className={`flex justify-between text-sm dark:text-secondary_text text-[#5E5E5E] mt-auto  ${gamePage ? "max-[550px]:hidden" : ""}`}>
