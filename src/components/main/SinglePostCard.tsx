...
import PostNewLabel from "./PostNewLabel";
...
const SinglePostCard = ({
  ...
  if (!gridView) {
    return (
      <Link
        href={
          cardType === "pc-games" ? item.slug || "" : "/games/" + item.title
        }>
        <div
          key={item.title}
          className={`flex flex-row items-center gap-4 p-4 h-[80px] border-b border-x border-border/40
            ${isFirst ? "rounded-t-lg border-t" : ""} 
            ${isLast ? "rounded-b-lg" : ""}
            dark:bg-bg dark:hover:bg-bg/80 bg-white hover:bg-white/60 
            transition-colors duration-300 ease-in-out`}>
          <div className="h-full aspect-square relative">
            <ImageWithFallback
              src={item.images[0]}
              alt={item.title + " image"}
              className="object-cover rounded-md group-hover:scale-105 scale-100 transition-all duration-300 ease-in-out dark:opacity-80 opacity-90"
              fill
              priority
            />
          </div>

          <PostNewLabel item={item} gridView={gridView} />

          <div className="flex flex-grow justify-between items-center">
            <div className="flex-grow">
              <h3 className="text-lg mb-0 font-semibold !text-[15px] line-clamp-1 dark:text-[#F8FAFC] text-[#292929]">
                {item.title}
              </h3>
            </div>

            <div className="flex gap-8 text-sm dark:text-secondary_text text-[#5E5E5E] pl-3">
              ...
              <span className="flex items-center max-[745px]:hidden">
  ...
  if (cardType === "pc-games") {
    ...
    return (
      <Link href={"/games/" + item.title}>
        <div
          key={item.title}
          className={`dark:bg-bg overflow-hidden border transition-colors duration-300 ease-in-out border-border/40 hover:border-border  w-full h-full px-2 py-8 dark:hover:bg-bg/90 bg-white hover:bg-[#f5f5f5] group max-[600px]:py-6 max-[600px]:px-[6px] rounded-lg `}
          onMouseEnter={() => setIsHovering(true)}
          onMouseLeave={() => setIsHovering(false)}>
          <PostNewLabel item={item} gridView={gridView} />
          <div
            className={`flex  justify-center items-center w-full mb-5 relative`}>
          ...
          <div className=" flex flex-col text-center">
            ...
            <h3 className="text-[19px] font-bold mb-[6px] px-1">
              {item.title}
            </h3>
            <div className="">
              <p className="text-secondary_text text-[13px] line-clamp-2">
                {item.cardDescription}
              </p>
            </div>
          </div>
